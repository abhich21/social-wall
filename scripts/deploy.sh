...
for i in $(seq 1 15); do
    ...
    if docker exec social-wall-rembg python -c "import urllib.request; urllib.request.urlopen('http://127.0.0.1:5000/health', timeout=3)" >/dev/null 2>&1 \
      && docker exec social-wall-app wget -qO- http://localhost:3000/health >/dev/null 2>&1; then
